...
inline int le_memcmp(const void *a, const void *b, size_t size) {
  int i;
  unsigned char *aa, *bb;
  aa = (unsigned char *)a;
  bb = (unsigned char *)b;
...
inline int be_memcmp(const void *a, const void *b, size_t size) {
  int i;
  unsigned char *aa, *bb;
  aa = (unsigned char *)a;
  bb = (unsigned char *)b;
...
size_t align_to_page(size_t len, int up) {
...
void print_help(char *name) {
  printf(
      ...
      "    allocate terabytes or more if you tell it to. Default: " 
      AS_STRING(STREAM_MATCH_LENGTH) " bytes.\n"
      "\n"
      "  -p <int>\n"
...
const char *eq_to_str(equality_t eq) {
  switch (eq) {
  case EQ:
    return "=";
    break;
...
equality_t str_to_eq(char *str, char **tail) {
  equality_t eq;
  switch (str[0]) {
  case '>':
    str++;
...
int print_skip_tokens(token_t *token, int print_parens) {
  ...
  case NE:
    ...
    break;
  default:
    fprintf(stderr, "encountered unexpected equality in parsed tokens: (%s)",
...
void print_token_metadata(token_t *tokens) {
  int t, i;
  t = 0;
  do {
    if (t > 0) {
...
unsigned char *prepare_token_bytes(token_t *token, token_t *previous_token,
                                   size_t n) {
  unsigned char *ptr;
  size_t tmp;
  ...
  if (token->skip_eq == NA &&
      ...
      previous_token != NULL && token->match_eq == previous_token->match_eq) {
...
#define MATCH_LOOP_BODY                                                        \
  ...
  } else if (match_tokens(tokens, t + 1, data,                                 \
                          tokens[t].match_offset + tokens[t].match_len,        \
                          local_offset_limit, global_offset, filename)) {      \
    return 1;                                                                  \
  }
...
#define MATCH(COND)                                                            \
  if (big_endian) {                                                            \
    for (i = start; i <= end; i++) {                                           \
      if (be_memcmp(data + i, tokens[t].match_seq, tokens[t].match_len)        \
              COND) {                                                          \
        MATCH_LOOP_BODY                                                        \
      }                                                                        \
    }                                                                          \
    ...
    for (i = start; i <= end; i++) {                                           \
      if (le_memcmp(data + i, tokens[t].match_seq, tokens[t].match_len)        \
              COND) {                                                          \
        MATCH_LOOP_BODY                                                        \
      }                                                                        \
    }                                                                          \
...
#define MATCH_SWITCH                                                           \
  ...
  case LE:                                                                     \
    ...
    break;                                                                     \
  default:                                                                     \
    fprintf(stderr, "error: unhandled equality type in match token (%s)\n",    \
            eq_to_str(tokens[t].match_eq));                                    \
    return 1;                                                                  \
  }

inline int match_tokens(token_t *tokens, int t, unsigned char *data,
                        size_t local_offset, size_t local_offset_limit,
                        off_t global_offset, char *filename) {
  int big_endian;
...
int search_file(char *file, token_t *tokens, char *range_string) {
  ...
  data = mmap(NULL, range.length + tmp_offset, PROT_READ, MAP_PRIVATE, fd,
              global_offset);
  ...
  rc = match_tokens(tokens, 0, data, range.start, range.start + range.length,
                    global_offset, file);
...
inline size_t fill_buffer(unsigned char *data, size_t bufsize) {
...
int search_stream(FILE *fd, token_t *tokens, char *range_string,
  ...
  if (tokens[0].min_len > stream_match_limit) {
    fprintf(stderr,
            "error: the minimum match length exceeds the maximum match length "
            "limit\n");
    return 1;
  }
  ...
  } while ((!range.length || remaining > tokens[0].min_len) && used >= bufsize);
...
token_t *parse_args(int argc, char **argv, int *rc) {
  int i, j;
...
int main(int argc, char **argv) {
  ...
  if (OPTIONS.file) {
    ...
    rc = search_stream(stdin, tokens, OPTIONS.range_string,
                       OPTIONS.stream_match_limit);
  }
  free_tokens(tokens);
